...
- Passes through classified auth-reject diagnostics (`diagnostics.ingress.auth_rejected_total`, `diagnostics.ingress.auth_rejected_by_reason`) and the runtime-secret rollup (`diagnostics.runtime_secrets`: `pools_without_valid_version`, `pools_without_valid_version_names`, `pools_total`, and per-pool version counts and expiry deadlines) when present in Admin health details. A non-zero `pools_without_valid_version` means at least one route is rejecting every webhook it receives.
