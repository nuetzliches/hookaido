...
A release about the rejects nobody could attribute. Hookaido answered `401` —
or `429`, or `503` — and every observability surface it owns reported the fact
without the cause or the route: a runtime secret pool that had run out of valid
versions looked exactly like a sender signing with the wrong key, and a
throttled route looked exactly like a busy instance. In the production incident
that started this, a pool ended up empty after a deploy while the issuer that
fills it runs on a daily schedule; every webhook in between was rejected, the
listener was up, `config validate` was green, `/healthz` was green, and the only
record of the outage was in a third party's delivery log. It closes
[#295](https://github.com/nuetzliches/hookaido/issues/295) and the
route-attribution follow-up that issue's own PR had deferred.

Nothing in the process observed the moment a secret version lapsed, so the
transition into the broken state was silent by construction — which is why the
fix is four surfaces reading one census rather than one new metric. A pool with
no currently-valid version now shows up in the Prometheus gauges, in
`/healthz?details=1`, in a WARN at startup and on every reload, and in a WARN on
the sweep that observes it happening to a running process. On the request side,
the single `reason="auth"` bucket was split into causes with different owners:
`no_valid_secret` is a Hookaido-side outage affecting every sender,
`signature_mismatch` and `timestamp_out_of_window` are one sender misbehaving.
Every ingress reject now also carries the route it happened on, closing the same
multi-route blind spot for rejects that v2.14.0 closed for backlog.

### Added

- **A runtime secret pool with no valid version is visible before the senders notice.** A route whose `secret_ref` pool held no version valid *right now* answered `401` to every inbound webhook, and validity is a function of the wall clock — so `Pool.Size()` could not tell a pool in the middle of a healthy rotation apart from one whose last version had lapsed. Four surfaces now report it, all reading the same `Pool.StateAt` census so they cannot disagree: the gauges `hookaido_runtime_secret_pool_versions{pool,state}` (`valid`/`pending`/`expired`), `hookaido_runtime_secret_pool_next_expiry_seconds{pool}` and `hookaido_runtime_secret_pool_exhaustion_seconds{pool}`, censused at scrape time and covering static `secret` blocks too, since a static pool whose one version has lapsed fails identically; a `runtime_secrets` rollup under `GET /healthz?details=1` whose `pools_without_valid_version` is the one number a plain HTTP uptime checker can assert on without a Prometheus scrape, with per-pool counts and both deadlines as absolute timestamps *and* countdowns; `runtime_secret_pool_without_valid_version` at WARN on the sweep that observes the transition, with `runtime_secret_pool_valid_version_restored` at INFO on recovery — transition-based rather than state-based, because a WARN every five minutes for a day-long outage is a line operators learn to filter; and `route_secret_ref_without_valid_version` at WARN at startup and on every reload, naming the route, the empty pools and how many usable secrets are left, since a route carrying a static secret beside an empty pool loses its rotation overlap but not its ability to authenticate. Alert on `exhaustion_seconds`, not on `next_expiry_seconds` — the latter drops to near zero on every handover of a perfectly healthy overlapping rotation, while the former moves only when the last credential is about to go. ([#295](https://github.com/nuetzliches/hookaido/issues/295))

- **Auth rejects carry a classified cause and the route they happened on: `hookaido_ingress_auth_rejected_total{route,reason}`.** `no_valid_secret`, `signature_mismatch`, `timestamp_out_of_window`, `replay`, `malformed`, `credentials` and `forward_denied` were previously one indistinguishable `reason="auth"` bucket with no route attribution, so diagnosing a `401` meant guessing between three problems with three different owners — an empty secret pool is a Hookaido-side outage rejecting *every* sender, a signature mismatch is usually one sender that has not adopted a rotation, and a timestamp outside the tolerance is clock skew. `internal/ingress` grew classified sentinel errors that all wrap `ErrUnauthorized`, so `errors.Is(err, ErrUnauthorized)` stays the correct test for "not authorized" and no caller has to enumerate causes; `QueryAuth` gained `VerifyCause`, which exposes the fail-closed branch it already had but could not report. Nothing about any response changed: every path that answered `401` or `404` still does, with the same body, and no secret material reaches a label — the cause says *why* it refused, never *what* was presented. `diagnostics.ingress.auth_rejected_by_reason` carries the same breakdown folded across routes. ([#295](https://github.com/nuetzliches/hookaido/issues/295))

- **Every ingress reject names the route it happened on.** `hookaido_ingress_rejected_by_reason_total` gained a `route` label, so a throttled, overloaded or policy-rejecting route can be named rather than inferred — the same multi-route blind spot v2.14.0 closed for backlog, now closed for rejects: a low-volume route rejecting everything was numerically invisible beside a busy route's normal reject rate, and the reason alone could not say which sender to go and talk to. It is a label on the existing family rather than a family of its own, because each reject is counted under exactly one `(route, reason, status)` triple — so nothing is double-counted, unlike the queue *gauges*, where labeled and unlabeled series in one family would have doubled every `sum()`. Two rejects are taken before a route resolves and report `route=""`: an unmatched path (`404`), and a path that matches while its method does not (`405`, where several routes can match one path with different methods, so no single route can honestly be named). Cardinality is bounded by configuration and not by traffic — the label is fed from the compiled route path, never from the requested one. `hookaido_ingress_adaptive_backpressure_total{reason}` stays route-less by design: its `reason` is an instance-wide trigger condition, and which routes got shed is now in the reject family under `reason="adaptive_backpressure"`.

### Changed

- **`hookaido_metrics_schema_info` moves to `1.7.0`** — through `1.6.0` for the runtime-secret pool gauges and the classified auth-reject counter, then `1.7.0` for the `route` label on `hookaido_ingress_rejected_by_reason_total`. The label addition is backward compatible at the series level, because Prometheus does not distinguish an empty label value from an absent one: the zero-filled reason/status baseline is the same series it was, matchers like `{reason="auth"}` still select every route, and `sum by (reason) (...)` is unchanged. The one visible difference is that a selector which returned exactly one series per reason/status now returns one per route as well, so a panel plotting it raw gains a line per route — aggregate with `sum by (reason)` to restore the old shape. `/healthz?details=1` keeps folding rejects and auth causes across routes: a label costs a series, and a JSON document an uptime checker polls on a short interval is the wrong place to pay for one.
...
- **`hookaido_metrics_schema_info` moves to `1.5.0`** for the new families. The instance-global `hookaido_queue_depth`, `hookaido_queue_total`, `hookaido_queue_oldest_queued_age_seconds` and `hookaido_queue_ready_lag_seconds` are unchanged, deliberately: adding a `route` label to them would have put labeled and unlabeled series in one family, and every dashboard doing `sum(hookaido_queue_depth{state="queued"})` would silently have started reporting double. The breakdown lives in the separate `hookaido_queue_route_*` families instead, the way `hookaido_ingress_rejected_by_reason_total` sits beside `hookaido_ingress_rejected_total`, and `sum by (state) (hookaido_queue_route_depth)` reproduces the aggregate.

- **The Pull API consumer guidance says what a consumer must do about a dead stream.** The docs explained keepalives from the server's side — "for proxies, not for delivery" — which reads as though there is nothing for the consumer to do, while the consumer side is where the value is: a half-open connection returns no error and no EOF, so a stream that will never deliver again looks exactly like a quiet one. `docs/pull-api.md` now says to keep a read deadline of a small multiple of `sse_keepalive` (default 15s), treat a longer idle as a dead connection and reconnect, and to set `sse_max_connection` as defense in depth since it bounds how long such a state can persist at all. ([#289](https://github.com/nuetzliches/hookaido/issues/289))
...
- **Pull metrics carry a `consumer_group` label** and `hookaido_metrics_schema_info` moves to `1.4.0`. Without it a route with two groups and one consumer each would read `hookaido_pull_sse_connection_active{route=...} 2` — the expected state — and an unexpected third consumer would be invisible, which would quietly undo the diagnostic added alongside it. The label is empty for a route without consumer groups, and Prometheus treats an empty label value as absent, so every existing series keeps its identity and existing dashboards and alerts continue to match. The Admin API's health diagnostics key grouped queues as `route#group` under `pull.by_route`; an ungrouped route keeps its bare path there.
...
- **`--watch-interval` gives `--watch` a polling fallback, and a single-file mount now warns instead of failing silently.** `--watch` watches the parent directory and filters by basename, which is correct for editors and atomic replace-by-rename — and cannot fire at all in the most common container shape, the config mounted as a single file. With a single-file bind mount the directory inside the container is not the host directory; it holds one bind-mounted entry, and replacing the file on the host creates a new inode the mount does not resolve to. The container's directory genuinely did not change, so there is no event to receive. Kubernetes `subPath` ConfigMap mounts have the same property. The failure was silent: `watching_config` was logged, no reload happened, and a new route stayed `404` while the operator believed the config was live. Two changes: `--watch-interval 30s` (off by default, minimum `1s`, requires `--watch`) re-reads the config and reloads when its content hash changes, through the same path as an fsnotify event; and without it, Hookaido logs `watch_may_not_fire` at startup on Linux when the config file turns out to live on a different filesystem than its own directory — the signature of a single-file mount. `docs/docker.md` now mounts the config directory rather than the file. ([#275](https://github.com/nuetzliches/hookaido/issues/275))

- **`auth query "<param>" <secret>` authenticates event sources that can only be given a URL.** Some sources can be handed nothing but a URL — no custom header, no signing secret, no basic-auth credentials — which is common with telephony/PBX platforms, appliance webhooks and older ERP systems whose configuration UI has a single "URL" field. All three existing route `auth` variants need something such a source cannot supply, so the only way through was to abuse `match query` as a credential check. That worked, and it kept useful properties, but it was a matcher rather than an authenticator: the route reported as having no authenticator at all to anyone auditing the config, and it could not reach `secret_ref` or the runtime secrets pool, so the token could only be rotated by editing the config. The new variant keeps every property that made the workaround the least-bad option — a failure answers `404` rather than `401`, so it does not confirm the path exists; verification happens before the rate limiter and before the body is read, so a wrong token costs no queue work; and the token never reaches the access log, the envelope, the queue, metrics labels or the delivery target — and adds constant-time comparison over SHA-256 digests, `secret_ref` support with several accepted tokens at once for rotation overlap, and a compile error for a parameter with no usable secret. Mutually exclusive with `auth basic`/`auth hmac`/`auth forward`, and it rejects a `match query`/`query_exists` on the same parameter. **A URL token is not replay-safe** — there is no nonce and no timestamp — which is a property of the source, not of Hookaido; the docs now say so in one place instead of leaving every operator to rediscover it, along with why a token in the path is worse. ([#273](https://github.com/nuetzliches/hookaido/issues/273))
