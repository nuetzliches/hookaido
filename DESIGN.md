...
- Scope guardrail (project-wide): Worker gRPC stays limited to pull-worker lease operations (`dequeue`, `ack`, `nack`, `extend`) and does not become an admin/publish/control-plane surface.
- MCP coverage decision (project-wide): no Worker-lease MCP tools; lease control remains on Pull/Worker runtime transports only.
...
- Metrics include ingress counters: `hookaido_ingress_accepted_total`, `hookaido_ingress_rejected_total`, `hookaido_ingress_rejected_by_reason_total{route,reason,status}`, `hookaido_ingress_enqueued_total`, adaptive-backpressure counters `hookaido_ingress_adaptive_backpressure_total{reason}`, and `hookaido_ingress_adaptive_backpressure_applied_total`.
- Ingress reject reason labels include `memory_pressure` for retained-footprint admission rejects in the memory backend.
- Every ingress reject is attributed to the route it happened on via the `route` label. It is empty for the two rejects taken before a route resolves -- an unmatched path (`404`) and a path whose method does not match (`405`, where several routes may match one path with different methods, so no single route can be named). Each reject is counted under exactly one `(route, reason, status)` triple, so `sum by (reason)` still reproduces the instance-wide count; only `reason` and `status` are zero-filled. `hookaido_ingress_adaptive_backpressure_total{reason}` stays route-less because its reason is an instance-wide trigger condition.
- Auth rejects are additionally counted as `hookaido_ingress_auth_rejected_total{route,reason}`, where `reason` is one of `no_valid_secret`, `signature_mismatch`, `timestamp_out_of_window`, `replay`, `malformed`, `credentials`, `forward_denied`, `unspecified`, `other`. The coarse `reason="auth"` bucket keeps its meaning and its total; the sum over this family equals it. `no_valid_secret` is the one cause that is a Hookaido-side outage rather than a sender problem, which is why it is separated.
- Metrics include on-scrape runtime-secret pool gauges for every registered pool, static blocks included: `hookaido_runtime_secret_pool_versions{pool,state}` (`valid`, `pending`, `expired`), `hookaido_runtime_secret_pool_next_expiry_seconds{pool}` and `hookaido_runtime_secret_pool_exhaustion_seconds{pool}`. Validity is a function of the wall clock, so these are censused at scrape time rather than accumulated from observations. Both deadline gauges report `+Inf` when no deadline applies and `0` when the pool holds no valid version. A process declaring no secrets emits none of the three.
- A pool with no version valid at the check time is reported as `runtime_secret_pool_without_valid_version` (WARN, on the sweep observing the transition; `runtime_secret_pool_valid_version_restored` at INFO on recovery) and as `route_secret_ref_without_valid_version` (WARN, per affected route, at startup and on every reload). `GET /healthz?details=1` carries the same state under `diagnostics.runtime_secrets`.
...
- Metrics include on-scrape queue gauges: `hookaido_queue_depth{state}` (queued, leased, dead), `hookaido_queue_total`, `hookaido_queue_oldest_queued_age_seconds`, and `hookaido_queue_ready_lag_seconds`, plus the per-queue `hookaido_queue_route_depth{route,consumer_group,state}`, `hookaido_queue_route_oldest_queued_age_seconds{route,consumer_group}` and `hookaido_queue_route_ready_lag_seconds{route,consumer_group}`.
- Pull metrics (`hookaido_pull_*`) carry a `consumer_group` label alongside `route`. It is empty for a route without consumer groups, which Prometheus treats as an absent label, so existing series keep their identity; a route that fans out to groups stays separable, which is what keeps the connection gauge meaningful there. Emitting it raised `hookaido_metrics_schema_info` to `1.4.0`; the per-queue backlog families raised it to `1.5.0`, and the runtime-secret pool gauges plus `hookaido_ingress_auth_rejected_total{route,reason}` raised it to `1.6.0`, and the `route` label on `hookaido_ingress_rejected_by_reason_total` raised it to `1.7.0`.
