...
This avoids interpreting missing pre-`1.3.0` series as zero. (The schema is `1.5.0` since the queue gauges gained per-route families; the adaptive series themselves are unchanged, so gate on whichever version you actually require.)
