...
ADAPTIVE_MAX_READY_LAG_SECONDS ?= 30
ADAPTIVE_MAX_OLDEST_QUEUED_AGE_SECONDS ?= 30
ADAPTIVE_MAX_READY_LAG_DELTA_SECONDS ?= 10
...
adaptive-ab-lag-guardrail-check:
	@test -n "$(RUN_ROOT)" || (echo "RUN_ROOT is required, e.g. make adaptive-ab-lag-guardrail-check RUN_ROOT=.artifacts/adaptive-ab/<run-id>" && exit 1)
	"$(ADAPTIVE_LAG_GUARDRAIL_SCRIPT)" \
		--run-root "$(RUN_ROOT)" \
		--scenario mixed \
		--max-ready-lag-seconds "$(ADAPTIVE_MAX_READY_LAG_SECONDS)" \
		--max-oldest-queued-age-seconds "$(ADAPTIVE_MAX_OLDEST_QUEUED_AGE_SECONDS)" \
		--max-ready-lag-delta-seconds "$(ADAPTIVE_MAX_READY_LAG_DELTA_SECONDS)" \
		--max-oldest-queued-age-delta-seconds "$(ADAPTIVE_MAX_OLDEST_QUEUED_AGE_DELTA_SECONDS)" \
		--min-accepted-total "$(ADAPTIVE_LAG_MIN_ACCEPTED)"

adaptive-ab-mixed-lag-guardrail:
	@RUN_ID="$$(date -u +%Y%m%d-%H%M%S)"; \
	"$(ADAPTIVE_AB_SCRIPT)" \
		--scenario mixed \
		--output-root "$(ADAPTIVE_AB_ROOT)" \
