...
- **Every ingress reject now names the route it happened on.** `hookaido_ingress_rejected_by_reason_total` gained a `route` label, so a throttled, overloaded or policy-rejecting route can be named instead of inferred — the same multi-route blind spot the per-queue backlog gauges closed for backlog, and the classified auth causes closed for auth. The label is empty for the two rejects taken before a route resolves: an unmatched path (`404`) and a path whose method does not match (`405`, where several routes may match one path with different methods). Prometheus does not distinguish an empty label value from an absent one, so those series keep the identity they had, matchers like `{reason="auth"}` still select every route, and `sum by (reason) (...)` is unchanged — but a selector that returned one series per reason/status now returns one per route as well, so raw panels gain a line per route. `hookaido_ingress_adaptive_backpressure_total{reason}` stays route-less by design: its reason is an instance-wide trigger, and which routes got shed is in the reject family. `/healthz?details=1` keeps folding rejects across routes. Metrics schema is now `1.7.0`.
- **A runtime secret pool with no valid version is now visible before the senders notice.** A route whose `secret_ref` pool held nothing valid answered `401` to every inbound webhook while the listener, `config validate` and `/healthz` all stayed green — the only evidence was in the sender's own delivery log. Four surfaces now report it: the gauges `hookaido_runtime_secret_pool_versions{pool,state}`, `hookaido_runtime_secret_pool_next_expiry_seconds{pool}` and `hookaido_runtime_secret_pool_exhaustion_seconds{pool}` (censused at scrape time, and covering static `secret` blocks too); a `runtime_secrets` rollup under `GET /healthz?details=1` whose `pools_without_valid_version` is the one number an HTTP-only uptime checker can assert on; a `runtime_secret_pool_without_valid_version` WARN on the sweep that observes the transition, with an INFO line on recovery; and a `route_secret_ref_without_valid_version` WARN at startup and on every reload, naming the route and whether anything is left that can still authenticate. Alert on `exhaustion_seconds` rather than `next_expiry_seconds` — the latter drops to near zero on every handover of a healthy rotation. Closes [#295](https://github.com/nuetzliches/hookaido/issues/295).
- **Auth rejects carry a classified cause and the route they happened on.** `hookaido_ingress_auth_rejected_total{route,reason}` separates `no_valid_secret`, `signature_mismatch`, `timestamp_out_of_window`, `replay`, `malformed`, `credentials` and `forward_denied` — three unrelated problems with three different owners that were previously one `reason="auth"` bucket with no route attribution. The coarse bucket keeps its meaning and its total, so existing rules are unaffected; `diagnostics.ingress.auth_rejected_by_reason` carries the same breakdown folded across routes. Metrics schema is now `1.6.0`.
