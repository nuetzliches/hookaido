...
UPDATE queue_items
SET state = ?, lease_id = NULL, lease_until = NULL, next_run_at = ?, dead_reason = NULL, attempt = 0
WHERE state = ?
...
func (s *Store) shouldSweepExpiredLeases(now time.Time) bool {
	...
	lastNanos := atomic.LoadInt64(&s.lastLeaseSweepNanos)
	if nowNanos-lastNanos < int64(interval) {
		return false
	}
